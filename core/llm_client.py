...
def create_llm_client_from_config(config) -> LLMClient:
    ...
    force_ollama = os.getenv('FLYNT_FORCE_OLLAMA', '1') == '1'
    ...
    # Run a quick health check and emit helpful logs
    try:
        ...
        if f and not f.get('success'):
            logger.warning(f"Fallback LLM health check: {f.get('message')}")
        elif f and f.get('success'):
            logger.info(f"Fallback LLM healthy: {f.get('message')}")

    except Exception as e:
        logger.warning(f"LLM health check raised an exception: {e}")
